...
run_cf_cell <- function(r, y, seed, sample_frac, hhs_raw,
  ...
  solve_equilibrium <- function(cd_scenario, comm_sc, p_init,
    ...
    list(sol = sol, p = p_sol, mc = mc_eff, claims = mc_sol$predicted_claims, shares = shares_sol,
...
")
  ...
  if (!is.null(eq_obs)) {
    results_list[[length(results_list) + 1]] <- tibble(
      ...
      claims = eq_obs$claims[plan_ids_cell],
  ...
  for (tau in TAU_GRID) {
    ...
    if (!is.null(eq_tau)) {
      ...
      claims = eq_tau$claims[plan_ids_cell],
  ...
  if (!is.null(eq_unif)) {
    results_list[[length(results_list) + 1]] <- tibble(
      ...
      claims = eq_unif$claims[plan_ids_cell],
  ...
  for (sc in SCALE_GRID) {
    ...
    if (!is.null(eq_sc)) {
      ...
      claims = eq_sc$claims[plan_ids_cell],
  ...
  if (!is.null(eq_al)) {
    results_list[[length(results_list) + 1]] <- tibble(
      ...
      claims = eq_al$claims[plan_ids_cell],
  ...
  for (tau in ENDOG_TAU_GRID) {
    ...
    if (!is.null(eq_e)) {
      ...
      claims = eq_e$claims[plan_ids_cell],
  ...
  if (length(endog_prefixes) > 0) {
    ...
    if (!is.null(eq_fm)) {
      ...
      claims = eq_fm$claims[plan_ids_cell],
  ...
  for (df in DEFUND_GRID) {
    ...
    if (!is.null(eq_d)) {
      ...
      claims = eq_d$claims[plan_ids_cell],
