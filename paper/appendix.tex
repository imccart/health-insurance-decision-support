...
\paragraph{Choice probabilities.} The choice probabilities are given in equation~(\ref{eqn:choice_probs}) as
...
\paragraph{Price and commission derivatives.} The partial derivative of the choice probability with respect to the insurer's base premium is
...
\begin{equation*}
	...
	\noindent for all plans $j,k$, where
	

	
		\begin{eqnarray*}\label{demand_sensitivity_element}
		\frac{\partial q_{ikt}({\bf p},{\bf y})}{\partial  p_{ijt}} = \begin{cases} 
			\beta_i^p q_{ijt}({\bf p},{\bf y}) \left[\frac{1}{\lambda} + \frac{\lambda-1}{\lambda}q'_{ijt}({\bf p},{\bf y}) - q_{ijt}({\bf p},{\bf y})  \right]  & k = j \\		
			\beta_i^p q_{ijt}({\bf p},{\bf y}) \left[\frac{\lambda-1}{\lambda}q'_{ikt}({\bf p},{\bf y}) - q_{ikt}({\bf p},{\bf y})  \right] & k \neq j  
			
		\end{cases}
	\end{eqnarray*}


    \begin{eqnarray*}\label{comm_sensitivity_element}
		\frac{\partial q_{ikt}({\bf p},{\bf y})}{\partial  \kappa_f} = \begin{cases} 
			\beta^y a_{it}^B  \, y_{jt}^0  \, q_{ijt}({\bf p},{\bf y}) \left[\frac{1}{\lambda} + \frac{\lambda-1}{\lambda}q'_{ijt}({\bf p},{\bf y}) - q_{ijt}({\bf p},{\bf y})  \right]  & k = j \\		
			\beta_y a_{it}^B \, y_{jt}^0 \,  q_{ijt}({\bf p},{\bf y}) \left[\frac{\lambda-1}{\lambda}q'_{ikt}({\bf p},{\bf y}) - q_{ikt}({\bf p},{\bf y})  \right] & k \neq j  
...
$$
y_{jt} = \kappa_f y_{jt}^0
$$

\begin{equation}
\label{eqn:utility}
\begin{split}
U_{ijt}({\bf p},{\bf y}) = {} & \alpha_{it}\, p_{ijt}({\bf p}) + \beta^{C}\, y_{jt}\, a_{it}^B
...
\begin{equation*}
		\frac{\partial q_{ikt}({\bf p},{\bf y})}{\partial y_{jt}} = \sum_{l \in J_{mt}} \frac{ \partial q_{ikt}({\bf p},{\bf y})}{\partial p_{ilt}({\bf p})}\frac{ \partial p_{ilt}({\bf p})}{\partial p_{jmt}}
	\end{equation*}
...
\noindent In our notation, the official risk adjustment transfer  $RA_{ft}({\bf p}_t)$ as derived by \citet{Pope2014} equals
	...
	\noindent where the plan's expected average claims with adverse selection $\widehat{c}_{jmt}({\bf p}_t)$ and the plan's expected average claims without adverse selection $\widetilde{c}_{jmt}({\bf p}_t)$ are given by 
...
\begin{equation*}
		\frac{\partial R_{ft}({\bf p},{\bf y})}{\partial y_{jt}} = \sum_{i \in I, m \in M, k \in J_{fmt}} \mathbb{I}_{i,m,t} \sigma_{it} p_{kmt} q_{ikt}({\bf p},{\bf y})
\end{equation*}
...
The main text reports a single demand specification, the nested logit whose estimates appear there. Here we report how those estimates change as we add terms to the utility function and how they respond to the premium control function. Table~\ref{tab:app-demand-spec} reports four specifications, each estimated on the same 20\% sample and the same 114 rating-region-by-year cells as the main-text model. Column (1) is a nested logit with plan attributes and a single premium coefficient. Column (2) adds the demographic heterogeneity in the price coefficient and in metal preference. Column (3) adds the assistance and commission steering terms and is the specification used in the main text, reproducing those demand estimates exactly. Column (4) adds the agent-density control function.
