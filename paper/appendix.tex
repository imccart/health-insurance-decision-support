...
The body groups formal decision assistance into two categories, agents and navigators. Covered California's enrollment records distinguish a finer set of service channels, which we map into those categories. Table~\ref{tab:app-channels} lists the underlying codes, a short description of each, the category we group it into, and its share of enrollment records over 2014--2019.
...
Table~\ref{tab:app-dominated-sample} reports the body estimator re-run on new enrollees alongside the all-enrollee estimates. The body sample is all enrollees so that the design-based analysis and the structural model are estimated on the same households. Restricting to new enrollees produces larger effects for every channel, consistent with assistance mattering most for households making a first plan choice, and leaves the ordering across channels unchanged.
...
The main paper reports a single demand specification, the nested logit whose estimates appear there. This section reports how those estimates change as we add terms to the utility function and how they respond to the premium control function. Table~\ref{tab:app-demand-spec} reports four specifications, each estimated on the same 20\% sample and the same 114 rating-region-by-year cells as the body model. Column (1) is a nested logit with plan attributes and a single premium coefficient. Column (2) adds the demographic heterogeneity in the price coefficient and in metal preference. Column (3) adds the assistance and commission steering terms and is the body specification, reproducing the main-text demand estimates exactly. Column (4) adds the broker-density control function.
...
Inertia is a well-documented feature of health insurance plan choice, in employer coverage \citep{handel2013}, in Medicare Part D \citep{polyakova2016}, and in the ACA exchanges we study \citep{saltzman2025}. We do not model it, and this section explains why our estimates of decision assistance and commission steering are nonetheless reliable.

The first point is empirical. New enrollees have no prior plan and so carry no inertia by construction. Table~\ref{tab:app-dominated-sample} re-estimates the design-based effect of assistance on dominated-plan choice for new enrollees alone, and the effects are the same as in the full sample for every channel, if anything larger. The steering we estimate is therefore not an artifact of inertia, since it is present, and stronger, exactly where inertia cannot operate.

The second point is structural, and it is specific to a setting in which the persistent state is itself produced by steering. A household steered into a plan today tends to remain there tomorrow, so a commission has a direct effect on current choice and a further effect that compounds through the choices it produced in earlier periods. This has two implications. A static indicator for the prior-year plan is a poor control, since the prior plan lies on the causal path from past commissions to current choice, and conditioning on it removes the persistent steering rather than isolating a nuisance. And properly separating the direct and persistent channels would require a dynamic choice model, in which commissions in one period feed the inertia state in the next. That is a heavier object than the switching-cost model in \citet{saltzman2025}, which does not include commissions, because in our setting the persistent state is generated by the very mechanism we study.

We do not need that decomposition, because the quantity relevant for welfare is the total steering effect, direct plus persistent, and a static commission model already captures it. We show this with a simulation. In a two-period choice, commissions steer the first-period choice, and that choice persists into the second period through an inertia term, so a commission has both a direct effect and a compounding one. We then estimate two models on the second-period choices, a dynamic model with commissions and inertia and a static model with commissions only, and compare the own-commission marginal effect on the period-two share. Table~\ref{tab:app-comm-inertia} reports the result. The dynamic model recovers the true parameters and splits the effect into a direct commission coefficient and a large inertia coefficient, and its direct piece is only about half of the effect a commission has once persistence is included. The static commission model, which does not separate the channels, recovers the full cumulative effect, matching the dynamic model's total to within a thousandth of a share point. We verify in the same exercise that this holds out of sample, so that the static model reproduces the full commission-removal counterfactual and not merely the local slope, and that it holds across inertia strengths and randomized plan attributes rather than at a single calibration.

The reading is that our static specification does not miss the role of inertia. It captures the steering whole, in the commission coefficient, rather than dividing it between a commission term and an inertia term as a dynamic model would.
