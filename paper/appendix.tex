...
Section 5 reports a single demand specification, the nested logit whose estimates appear in the main text. This section traces how those estimates build up as we enrich the utility function and shows that they are robust to the premium control function. Table~\ref{tab:app-demand-spec} reports four specifications, each estimated on the same 20\% sample and the same 114 rating-region-by-year cells as the body model. Column (1) is a nested logit with plan attributes and a single premium coefficient. Column (2) adds the demographic heterogeneity in the price coefficient and in metal preference. Column (3) adds the assistance and commission steering terms and is the body specification, reproducing the main-text demand estimates exactly. Column (4) adds the broker-density control function.

We summarize price sensitivity by the enrollment-weighted mean own-price elasticity rather than by the premium coefficient itself. The premium coefficient and the nesting parameter are jointly identified and trade off against each other as the controls accumulate, so neither is stable on its own. The nesting parameter rises from 0.38 to 0.67 across the build-up and the premium coefficient more than doubles in magnitude over the same columns, but the own-price elasticity they jointly imply sits between $-0.75$ and $-0.82$ throughout. The economic content of the price response is pinned by the data even though its parameterization is not. The elasticity is measured with respect to the net premium households pay, which is why it is inelastic, since most enrollees are heavily subsidized and face little premium at the margin.

We likewise report the assistance results as marginal effects on plan choice rather than as log-odds coefficients. Turning navigator assistance on for every household raises the share choosing a silver plan by 7.5 percentage points, and broker assistance raises it by 7.1 percentage points, against an unassisted silver share near one half. The two effects are close even though the broker metal coefficient is the larger of the two, because brokers are also less price-sensitive, which pulls against the cheaper silver plan and offsets part of the stronger steering. The commission-by-broker coefficient is 0.02.

The informative robustness comparison is between columns (3) and (4). Adding the control function leaves every quantity nearly unchanged, the own-price elasticity, the nesting parameter, and both assistance effects. The broker-density instrument is too weak at the level of variation that matters to separate the control function residual from the premium coefficient, as we document in the plan-choice results of Section~\ref{app:reduced_form}, so we read the two columns as a reparameterization rather than as independent evidence. \citet{saltzman2019} likewise finds that this control-function correction leaves the nesting parameter essentially unchanged, with only a small downward bias in premium sensitivity absent the correction.
...
\input{results/tables/demand_spec_sensitivity.tex}
\end{minipage}
\end{table}
...
\input{results/tables/cost_sharing_schedule.tex}
\end{minipage}
\end{table}
...
\end{equation}
with $P$ the annual premium and $\rho$ the coefficient of absolute risk aversion. The variance term is what gives cost-sharing depth welfare weight beyond its effect on expected out-of-pocket spending, so a household will pay more than the actuarial difference to move from a bronze plan to a platinum one.
...
\input{results/tables/assistance_channels.tex}
\end{minipage}
\end{table}
...
\label{tab:app-pbe}
\input{results/tables/pbe_robustness.tex}
\end{minipage}
\end{table}
...
The certified insurance agents present the full menu but may still tilt a household's attention within it, a soft counterpart to the plan-based enroller's hard restriction. We enter any such tilt through the commission coefficient in utility. To show that this does not distort the plan choices the model predicts, we simulate a gatekeeper process and confirm that our specification reproduces its choices.
...
\input{results/tables/menu_simulation.tex}
\end{minipage}
\end{table}
