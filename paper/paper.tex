...
Finally, we use the estimated model to evaluate counterfactual policies that fall into three general categories: 1) withdrawing decision assistance; 2) reshaping the commission schedule while leaving assistance in place; and 3) substituting public navigators for private agents. Withdrawing assistance raises the uninsured share by [TBD] percentage points, reshaping commissions moves welfare by [TBD] per member per year, and substituting one channel for the other changes coverage by [TBD]. We decompose the welfare change into the coverage margin, and the risk protection that coverage provides, and the plan-choice margin. [discuss results] Commissions do create a spillover, since insurers pass part of the commissions they pay into the premiums every enrollee faces, including the unassisted, and we report that spillover and the steering distortion separately from the value of the assistance channel. [discuss results]
...
Table~\ref{tab:summary-stats} reports summary statistics separately for assisted and unassisted households, and the two groups differ substantially. Assisted households have lower income and are larger. They skew older, with a larger share of near-elderly members aged 55 to 64 and a smaller share of young adults aged 18 to 34, and a somewhat lower share of male members. Assisted households are also more likely to be Asian and less likely to be Black. Finally, assisted households are less likely to select a dominated plan. [TBD: the group means behind each of these comparisons.]
...
We note that agent density has limited power to speak to selection within a rating region. The instrument varies only across region-year cells, so a specification that absorbs region fixed effects leaves it with essentially no variation, and within a region the number of licensed agents is close to a deterministic function of the year. When we cluster at the level at which the instrument actually varies, the partial $F$ statistic on agent density is 3.6, against a value of 7,586 when standard errors ignore the clustering entirely. We therefore report the control function and the region fixed effects as separate specifications rather than combining them, and we treat the stability of the assistance coefficients across the sequence, rather than the control function coefficient itself, as the evidence on selection.
...
Attaching the commission term to every plan in an agent-assisted household's choice set treats the household as selecting an agent and a plan together rather than in a fixed order. A household that is already aware of a few plans and consults an agent as it nears a decision, and a household that chooses an agent and a plan at the same time, both reduce to an agent-channel household whose choice over the full menu is tilted by the commissions attached to those plans. The alternative in which a household first commits to a particular agent and is then confined to the carriers that agent represents would require the identity of each household's agent and that agent's carrier contracts, neither of which we observe. Because commissions enter the choice over the entire menu, $\beta^{y}_{1}$ and $\beta^{y}_{2}$ measure the average pull of commissions on agent-assisted choices.

We do not model the agent's recommendation as a separate decision. An agent's recommendation likely reflects both the commission and the fit of the plan for the client, and the household follows that recommendation to a degree we do not observe. The commission terms capture the combined result, namely how the choices of agent-assisted households move with the commissions attached to the plans in their choice set. Separating the agent's problem from the household's would require data on agents, their carrier appointments, and their recommendations, none of which we observe. Finally, to introduce some limits to an agent's commission-based steering of plan choice, we allow for nonlinearity in the commission terms via a simple quadratic.

The demographic interactions with the premium and with actuarial value let price sensitivity and the valuation of coverage generosity vary with household composition. These are not a channel distortion but a feature of preferences, and they matter for the supply side. Without them the model sorts households across metal tiers almost entirely through the price coefficient, predicted enrollee composition varies too little across plans within a market, and the risk score equation in Section~\ref{subsec:supply} cannot separate the demographic composition of a plan's enrollees from its generosity.

Insurers set commissions either as a flat per-member-per-month amount or as a percentage of the unsubsidized premium. For percentage-based commissions, we convert to per-member-per-month equivalents using the plan's average unsubsidized premium. In practice, commission schedules exhibit remarkably little temporal variation. Of the ten insurers operating on the exchange during our sample period, seven maintained identical commission rates across all years. Only the three largest insurers (Anthem, Blue Shield, and Health Net) adjusted their rates, and these adjustments followed a monotone downward trend rather than the year-to-year reoptimization that a commission first-order condition would imply. We return to the insurer's commission choice in the counterfactuals of Section~\ref{sec:welfare}, where we let commissions re-optimize according to the first-order condition in Equation~\eqref{eqn:commission_foc}.\footnote{Health Net's switch from percentage-based to flat commissions in 2018 followed its acquisition by Centene and represents an organizational change rather than marginal optimization.}
...
\paragraph{Assistance and the enrollment margin.} Assistance is recorded on the enrollment record, so every household we observe using a navigator or an agent is by construction enrolled. Estimating Equation~\eqref{eqn:choice_probs} as written would therefore read that fact as a taste for coverage and load it onto the assistance terms, which are the steering estimates the paper turns on. We avoid this by letting assistance operate on which plan a household chooses without letting the household's realized channel determine whether it enrolls at all. Write the inclusive value of the exchange nest under channel state $c$ as
\begin{equation}
\label{eqn:inclusive_value}
	I^c_{it}({\bf p},{\bf y}) = \ln \sum_{k \in J_{mt}} e^{V^c_{ikt}({\bf p},{\bf y})/\lambda},
\end{equation}
where $c$ indexes the three states of being unassisted, using a navigator, or using an agent, and $V^c$ adds that state's assistance terms to the base utility. The plan choice conditional on enrolling uses the household's realized channel, while the enrollment probability uses the expected inclusive value across states,
\begin{equation}
\label{eqn:two_part}
...
\end{equation}
with $\pi_{ict}$ the household's probability of using channel $c$, estimated in a first stage from household characteristics and the density of licensed agents in its rating region. A household's enrollment probability therefore depends on the assistance it might receive rather than on the assistance we happen to observe it receiving, while every steering term remains in the within-nest choice. Setting $\pi$ degenerate at the realized channel and $V^c = V$ recovers the ordinary nested logit in Equation~\eqref{eqn:choice_probs}. The counterfactual kernels carry the same structure, so a policy that changes who is assisted moves coverage through $\bar{I}_{it}$ rather than through a level shift on the inside goods.
...
\end{equation}
% Evan (2026-08-25) objected to insurer terms in this equation: they limit an
% insurer's ability to change its enrollee risk, and CMS uses demographics and
% diagnoses with no insurer terms. The terms he saw were a workaround for scores
% backed out of transfers, which over-scored Molina; those are gone, replaced by
% the observed SRRT scores he supplied. Those scores are what motivate the terms
% now: platinum runs 1.2 at Kaiser to 7.3 on Health Net's PPO, and the pooled
% specification misses platinum by 12 to 37 percent in sample. The demographic
% slopes stay common, so composition still moves the score in the counterfactual
% and only the levels are freed (decisions.md, 2026-08-26 and 2026-09-18).
where the first term is a full set of insurer-by-metal effects, consistent with CMS estimating the risk model separately by metal level, and $s_{djmt}({\bf p},{\bf y})$ is the share of plan $j$'s enrollment in demographic group $d$, namely members aged 0 to 34, male, in a family household, and non-white. Metal effects are insurer-specific because observed risk scores differ sharply across insurers within a tier. Among platinum plans the filed scores run from 1.2 at Kaiser to 7.3 on Health Net's PPO, a spread that a single tier indicator assigns to every plan alike, leaving predicted claims and the risk adjustment transfer wrong plan by plan. The demographic slopes are common across insurers and tiers, so the equation continues to respond to enrollment composition in the counterfactuals and only the levels are freed. We estimate the regression by weighted least squares on the observed plan-year risk scores reported in the California rate filings, weighting by member months, and hold the coefficients fixed. When the fitted model enters the equilibrium, the demographic shares are computed by aggregating the estimated household choice probabilities from Equation~\eqref{eqn:choice_probs} rather than from observed enrollment, following the approach widely used in the hospital choice literature to construct market concentration measures \citep{kessler2000}, so that predicted enrollee risk moves with demand. We estimate average claims using the estimating equation  
\begin{equation}
...
% Evan (2026-08-25) objected to the commission wedge as "not economically
% meaningful, a fudge parameter, a target for referees." At that time it was a
% markup calibrated separately for every region-year-insurer cell, alongside a
% per-cell cost residual. Both are gone. It is now one level per carrier,
% eleven parameters on 61 conditions, estimated inside the GMM rather than
% calibrated, and it is the same object as the big-four insurer dummies already
% in the claims equation (decisions.md, 2026-09-05 and 2026-09-08). The
% off-exchange parity story that first motivated it was tested and dropped: a
% leverage slope came out negative where parity predicts positive, and the
% within-carrier residual tracks on-exchange book growth, which cannot enter
% the condition because it is an outcome of the steering being modeled. So the
% constant stays and the mechanism is left unclaimed.
We estimate the condition net of a constant $\delta_f$ for each carrier. The constant is common across a carrier's plans and years, so it is identified from the carrier's several commission-setting units and years rather than fitted to any one of them, and it plays the same role on the commission margin that the insurer indicators in the claims equation play on cost. It absorbs a carrier-level difference in the return to a commission dollar that the modeled terms do not account for. Carrier size accounts for [TBD] of its variation across carriers, and we do not attach it to a particular mechanism.

\subsection{Estimation}
\label{subsec:estimation}

We estimate the model sequentially in three stages. First, we estimate the demand parameters $\boldsymbol{\beta}$ and the nesting parameter $\lambda$ by maximum likelihood using the nested logit choice probabilities in Equation~\eqref{eqn:choice_probs}. We pool households across markets and years, estimating a single set of coefficients, and weight each household by its size (number of members). As in Section~\ref{subsec:causal-methods}, demand is estimated on a 20 percent random sample of households drawn within each rating region and year. This weighting ensures that predicted market shares correspond to member-level enrollment, which is the relevant unit for insurer revenue, risk pool composition, and the supply-side first-order conditions. Second, given the estimated demand parameters, we compute market shares, the elasticity matrices $\Omega$ and $\Omega^B$, and the risk adjustment transfer derivatives for each market-year cell. Third, we estimate the cost parameters by two-step generalized method of moments, following \cite{saltzman2019}. The risk score equation is estimated separately by weighted least squares and its coefficients are held fixed, so it contributes no moment. The moment conditions stack three blocks, namely the claims residuals from Equation~\eqref{eqn:avg_claims_estimation} in levels, one pricing first-order condition residual per plan-year from Equation~\eqref{eqn:pricing_foc} in dollars per member-month, evaluated at observed premiums with marginal cost given by the fitted claims and risk equations net of risk adjustment and reinsurance, and the commission first-order conditions, one per commission-setting unit and year, net of a carrier-level constant. The second-step weight matrix is the inverse of the estimated moment covariance rather than a block-diagonal scalar, which matters for precision as much as for point estimates.\footnote{Using a scalar weight understates the risk score standard errors substantially in our data. [discuss results]} We drop a plan-cell from the first-order condition block when its predicted within-cell share falls below 0.005, since the first-order condition is ill-conditioned as the share approaches zero.
...
Table~\ref{tab:demand-estimates} reports the demand estimates, obtained from the 20 percent sample of [TBD] households. The nesting parameter is \lambdaHat{}, which we check against the unit interval required by random utility maximization and against the standard logit boundary. [discuss results] The base premium coefficient is $-0.411$ per hundred dollars of monthly premium. Both channel premium interactions add to that sensitivity rather than offsetting it, by $-0.096$ for navigator-assisted households and $-0.026$ for agent-assisted households, so households choosing with assistance respond somewhat more to premium differences than households choosing on their own.

The steering terms line up with the design-based evidence. The linear commission coefficient is positive and the squared term is negative ([TBD] and [TBD], with standard errors of [TBD] and [TBD]), so agent-assisted households are more likely to enroll in plans paying higher commissions, holding premiums and plan characteristics fixed, and the pull of an additional commission dollar falls as the commission rises, reaching zero at \$\commTurningPoint{} per member per month. Evaluated at the mean commission and the mean price coefficient among agent-assisted households, a \$1 increase in monthly commission shifts their enrollment as much as a \$\commPremRatio{} reduction in net monthly premium. Because mean commissions (\$\meanCommission{} per member per month) are much smaller than mean net premiums (\$\meanNetPremium{}), the same comparison in percentage terms puts the commission elasticity of agent-assisted demand at \commPremElast{} of its premium elasticity. The channel interactions with actuarial value shift how much each channel's households value generosity, and the demographic interactions with actuarial value generate the sorting across levels of generosity that identifies the demographic terms in the risk score equation. [discuss results]

Table~\ref{tab:cost-estimates} reports the cost estimates, which follow \cite{saltzman2019} where the specifications overlap. The risk score equation places the level of predicted risk in a full set of insurer-by-metal effects, so two platinum plans offered by different carriers are assigned different risk even at the same enrollee composition, while the demographic shares enter with slopes common across insurers and tiers. [TBD: the estimated effects and demographic slopes, and their precision.] In the claims equation, the pass-through of the risk score into average claims is 1.57, so a plan whose enrollees are ten percent riskier carries roughly sixteen percent higher claims.

Table~\ref{tab:supply-results} reports the recovered markups by metal tier. Markups are smallest on bronze plans and largest on gold, averaging \$\meanMarkup{} per member per month across plan-cells, a Lerner index of \meanLerner{}. Figure~\ref{fig:mc-validation} plots the marginal cost implied by the pricing first-order conditions against the structural prediction from the risk score and claims equations. [TBD: the correlation between the two, the number of plan-cells above the share floor, and how many carry negative net-of-transfer marginal cost.] The equilibrium comparisons in Section~\ref{sec:welfare} therefore depend on the fit of the cost model, and we report the distance between the model's baseline and observed premiums and commissions as the fit check.
...
\paragraph{Baseline.} Every scenario is measured against the model's own equilibrium at the observed commission schedule, in which premiums solve the pricing conditions and, for each commission-setting unit whose condition is identified, the commission scale solves the commission condition.
...
We solve this system of nonlinear equations by Broyden's method with a Levenberg-Marquardt hook step, starting from observed premiums or, where an earlier solve of the same year under the same estimates is available and starts closer, from that fixed point. Markets containing near-substitute plans produce Jacobians with condition numbers on the order of $10^6$, and the hook step is what keeps those cells stable.

% Evan holds the first-order-condition residual fixed at the observed point in
% every counterfactual (run.counterfactuals.jl, zero_resid_flag). We solve the
% conditions outright instead, so the baseline is the model's own equilibrium
% and the residual is reported rather than imposed (decisions.md, 2026-08-29).
...
\end{equation}
% Evan (2026-08-25) asked for household-specific premium parameters in consumer
% surplus rather than a weighted mean. Adopted 2026-08-26 (decisions.md): the
% denominator is each household's own base slope, which is fixed across
% scenarios because the channel interactions are excluded, so his point is met
% without the denominator moving between scenarios. Every household's base
% slope lies between -0.30 and -0.035 per $100, so no trimming is needed.
...
We begin with the effect on coverage. When commissions are banned and the formerly agent-assisted households receive no substitute ($\tau = 0$), the uninsured share rises by [TBD] percentage points. This shift does not depend on how we value being uninsured, and it points in one direction, since a household that loses coverage is worse off on access, health, and financial protection. When the withdrawn agents are replaced by navigators, the uninsured share sits [TBD] percentage points above the observed level. [discuss results] The coverage loss therefore comes from the withdrawal of agent assistance itself rather than from the removal of commissions. Consumer surplus moves in the same direction, falling sharply when assistance is withdrawn and recovering most of the way when agents are replaced by navigators, consistent with households valuing the guidance in their own choices. The navigator rule diverges from these two measures. It falls most when assistance is removed, since households left unassisted make the choices a well-informed advisor would steer them away from, but it rises when agents are replaced by navigators, since that substitution removes the commission steering while preserving the guidance.

The welfare value of that coverage loss depends on how heavily we weight the cost of being uninsured. At the lightest weighting, we value the uninsured option only at an estimated out-of-pocket cost from the MEPS, which falls well below the full cost of care since a large share of medical care for this population is ultimately uncompensated. On this basis, removing assistance is a welfare gain of roughly [TBD] per member per year under the objective measure, which values households by premium, expected out-of-pocket cost, and risk rather than by revealed preference, because the premium savings to households that remain insured exceed this modest out-of-pocket cost. Adding the value of risk protection and the mortality cost reverses this result. At the central valuation the objective effect is a loss of roughly \$[TBD] per member per year, and it ranges from a small gain of about \$[TBD] when these consequences are valued at the low end to a loss of about \$[TBD] at the high end. The width of this band, and the sign of the effect within it, reflect the mortality valuation almost entirely, so that removing assistance is a welfare loss at the central and high valuations and a slight gain only when the health consequences of losing coverage are valued at the low end.


We turn to reforms that change the commission schedule without withdrawing assistance. [discuss results] A uniform commission, commissions aligned with consumer value, and a flat-fee mandate for the percentage-based insurers each move the objective measure by less than [TBD] per member per year and leave the coverage share essentially unchanged, and the scaled-commission scenarios show the effect growing smoothly with the level of compensation. We report the welfare cost of the differential component of commissions apart from the agent channel itself. [discuss results]

We turn next to the scenarios in which insurers re-optimize commissions, which speak most directly to policy. Expanding navigator access alongside a functioning agent market changes the uninsured share by [TBD] and the objective measure by [TBD] per member per year at the central valuation. [discuss results] Defunding navigators and shifting those households to agents, the direction of the federal funding cuts between 2017 and 2019, lowers the uninsured share by [TBD] percentage points and raises the objective measure by about [TBD] per member per year. The gain from defunding grows with the assumed cost of being uninsured, since its main effect is to reduce the uninsured share, and it becomes a small loss only at the low end of the band. The navigator rule moves the other way, since the households shifted to agents now make more of the steered choices that rule penalizes, so whether defunding improves welfare depends on which measure one adopts.

% Evan (2026-08-25) asked for subsidies, cost-sharing reductions, the mandate
% penalty and uncompensated care. Only the premium tax credit is in. The
% penalty is already in the data and the CSR cost is computable from the
% cost-sharing table, so both could be added; uncompensated care needs an
% assumption we have not made (decisions.md, 2026-08).
Table~\ref{tab:cf-fiscal} reports the producer-surplus and government-cost sides of the same scenarios. Government cost here is the advance premium tax credit actually paid, and excludes cost-sharing reduction payments, the individual mandate penalty, and the cost of uncompensated care for those who end up uninsured. Because the credit is paid only on enrolled households, it tracks the coverage effect, falling when assistance is withdrawn and rising when households are shifted toward agents. The producer-surplus column shows the corresponding change in insurer margins net of the commissions that fund the agent channel.

Taken together, the counterfactuals point to one conclusion. The primary effect of decision assistance is to keep households insured. Removing it raises the uninsured share substantially, and whether this amounts to a large welfare loss or a small one depends on the cost assigned to being uninsured, which we report as a range rather than a single figure. We compare the commission distortions that motivate our analysis against the existence of the assistance channel itself. [discuss results]
