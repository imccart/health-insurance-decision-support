...
We begin by examining whether decision assistance affects plan choice and, if so, whether it improves the quality of decisions. Our estimand is the average treatment effect on the treated (ATT) for households receiving decision assistance, estimated on all enrollees so that the sample matches the structural analysis in Section~\ref{sec:structural}. We embed a conditional logit discrete choice model into a potential outcomes framework \citep{rubin1974, imbens2009}, following \cite{saltzman2019}.
