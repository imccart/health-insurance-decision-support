...
\label{eqn:utility}
\begin{split}
U_{ijt}({\bf p},{\bf y}) = {} & \alpha_{it}\, p_{ijt}({\bf p}) + \beta^{y}\, a_{it}^B\, y_{jt}
...
Each of the two assistance channels (agents and navigators) enters utility separately, allowing navigators and agents to shift metal tier preferences by different amounts. The commission term $\beta^{C}  a_{it}^B y_{jt}$ enters utility only for agent-assisted households because navigators do not receive plan commissions.  A positive commission parameter $\beta^{C}$ indicates that agents steer consumers toward plans with higher commissions. For unassisted households, all assistance-related terms are zero and choices are driven entirely by premiums, plan characteristics, and unobserved preferences.
...
\end{equation}
where $R_{ft}({\bf p},{\bf y})$ is total premium revenue, $C_{ft}({\bf p},{\bf y})$ is total claims,  $Y_{ft}({\bf p},{\bf y})$ is total commissions paid, $RA_{ft}({\bf p},{\bf y})$ is the net risk adjustment transfer, $RI_{ft}({\bf p},{\bf y})$ is reinsurance received from the ACA's transitional reinsuarance program, and $VC_{ft}({\bf p},{\bf y})$ is variable administrative cost excluding commissions. 

Formulas for all terms in equation~(\ref{eqn:profit}) are in Appendix~\ref{app:formulas}. Computing total premium revenue is straightforward, only requiring the choice probability estimates in equation~(\ref{eqn:choice_probs}).  For total firm claims, we also need estimates of average plan claims.  We estimate average claims using the estimating equation  
\begin{equation}
\label{eqn:avg_claims_estimation}
	\ln c_{jmt}({\bf p},{\bf y}) = \mu^r \ln \hat{r}_{jmt}({\bf p},{\bf y}) + x'_j  \mu^{x}  + \mu^l l_t + \epsilon_{jmt}^c,
\end{equation}
where $\hat{r}_{jmt}({\bf p},{\bf y})$ is the fitted risk score from Equation~(\eqref{eqn:risk_score_estimation}) below, $x_j$ is a network type indicator, and $l_t$ is a linear trend. Equation~(\eqref{eqn:risk_score_estimation}) writes average claims as a function of enrollee risk This construction of enrollee average claims allows enrolle r

%Define $J_{ft}$ as the set of plans sold by firm $f$ at time $t$ and $I$ as the set of households.  Total commissions paid by the firm are
%begin{equation*}
...
\end{equation}
for each plan $j$ offered by firm $f$. Formulas for each term in equation~(\ref{eqn:pricing_foc}) are given in Appendix~\ref{app:formulas}.  Observe that commissions affect every term in (\ref{eqn:pricing_foc}) through demand.
...
\noindent Formulas for each term in equation~(\ref{eqn:commission_foc}) are given in Appendix~\ref{app:formulas}.
