...
\label{eqn:utility}
\begin{split}
U_{ijt} = {} & \alpha_{it} \cdot p_{ijt} + \beta^{C} \cdot C_{jt} a_{it}^B
        + MT'_j \cdot \left( \beta^{N} \cdot a_{it}^N + \beta^{B} \cdot a_{it}^B \right) \\
        & + MT'_j \cdot \Gamma \cdot w_{it}
        + x'_{ij}\cdot \beta^x + \xi_{j} + \varepsilon_{ijt},
\end{split}
\end{equation}
...
% \centering
% \begin{minipage}[h]{6in}
% \caption[]{\textbf{Welfare Gradient}\footnote{Consumer surplus change as a function of broker-to-navigator substitution rate ($\tau$). Each point represents the mean change in consumer surplus (relative to the observed equilibrium) across all region-year cells at a given $\tau$.}}
% \centerline{%
...
% \centering
% \begin{minipage}[h]{6in}
% \caption[]{\textbf{Average Premium Change by Counterfactual Scenario}\footnote{Bars show the share-weighted mean change in posted premiums across region-year cells. Scenarios vary the commission structure and the broker-to-navigator substitution rate.}}
% \centerline{%
